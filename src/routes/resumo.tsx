...
function Resumo() {
  const { settings } = useLucro();
  const liquido = DIA.bruto - DIA.custos;
  const progresso = Math.min(100, (liquido / settings.dailyGoal) * 100);

  const barras = [
    { label: "Verdes", value: DIA.verdes, cls: "bg-go", text: "text-go" },
    { label: "Amarelas", value: DIA.amarelas, cls: "bg-warn", text: "text-warn" },
    { label: "Vermelhas", value: DIA.vermelhas, cls: "bg-stop", text: "text-stop" },
  ];

  return (
    <>
      <Screen title="Resumo do dia" subtitle="Terça-feira · 8h30 rodando">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          {[
            { l: "Ganho bruto", v: brl(DIA.bruto) },
            { l: "Custos do dia", v: brl(DIA.custos) },
            { l: "Corridas", v: String(DIA.corridas) },
            { l: "Lucro por hora", v: brl(liquido / DIA.horas) },
          ].map((k) => (
        ...
        <section className="glass-card mt-3 rounded-3xl p-5">
            ...
            {barras.map((b) => (
              <div key={b.label}>
                <div className="flex justify-between text-[11px] font-semibold">
                  ...
                  <span className="tabular-nums text-muted-foreground">{b.value} corridas</span>
                </div>
                <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-elevated">
                  <div
                    ...
                    style={{ width: `${(b.value / DIA.corridas) * 100}%` }}
                  />
                </div>
              </div>
            ))}
          ...
          <p className="mt-4 text-[11px] leading-relaxed text-muted-foreground">
            Você recusou 3 corridas vermelhas hoje e economizou cerca de {brl(41.7)} em prejuízo.
          </p>
        </section>
      </Screen>
      <BottomNav />
