    ...
    <section className="relative overflow-hidden pt-28 pb-16 md:pt-36 md:pb-24">
      <div className="absolute inset-0 -z-10 bg-gradient-to-br from-background via-background to-secondary">
        {company.hero_image_url && (
          <>
            <img src={company.hero_image_url} alt="Frota da UTIL LOCADORA" className="h-full w-full object-cover object-right" />
      ...
      <div className="container mx-auto px-4">
        ...
        <h1 className="mt-6 max-w-3xl font-display text-4xl font-extrabold leading-[1.05] tracking-tight text-foreground sm:text-5xl lg:text-7xl">
          {company.hero_title || "LOCAÇÃO DE CARROS PARA APLICATIVOS"}
        </h1>
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          <Link to="/cadastro" className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-7 py-4 font-bold text-primary-foreground shadow-lg shadow-primary/25 transition hover:brightness-95">
            QUERO ALUGAR UM CARRO <ArrowRight className="h-4 w-4" />
          </Link>
          <a href="#frota" className="inline-flex items-center justify-center rounded-full border border-border bg-background px-7 py-4 font-bold text-foreground transition hover:bg-secondary">
            VER NOSSA FROTA
          </a>
          <a href={whatsappUrl(company.whatsapp)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full border border-border bg-background px-7 py-4 font-bold text-foreground transition hover:bg-secondary">
            <MessageCircle className="h-5 w-5" /> FALAR COM A LOCADORA
          </a>
        </div>
        ...
        <div className="mt-14 grid grid-cols-2 gap-8 md:grid-cols-4">
          {stats.map((item) => (
            <div key={item.label}>
              <p className="font-display text-xl font-extrabold text-foreground">{item.value}</p>
