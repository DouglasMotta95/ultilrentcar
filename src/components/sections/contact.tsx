...
export function Contact() {
  ...
  const infos = [
    company.phone ? { icon: Phone, label: company.phone } : null,
    company.email ? { icon: Mail, label: company.email } : null,
    location ? { icon: MapPin, label: location } : null,
    company.hours ? { icon: Clock, label: company.hours } : null,
  ...
  return (
    <section id="contato" className="bg-surface py-20 md:py-28">
      <div className="container mx-auto px-4">
        ...
        <h2 className="mt-6 font-display text-3xl font-extrabold tracking-tight sm:text-4xl lg:text-5xl">Solicite seu orçamento agora</h2>
        ...
        <div className="mt-10 space-y-4">
          {infos.map(({ icon: Icon, label }) => (
            <div key={label} className="soft-card flex items-center gap-5 px-6 py-4">
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-accent text-primary"><Icon className="h-5 w-5" /></span>
        ...
        <div className="mt-10 flex flex-col gap-3 sm:flex-row">
          <a href={whatsappUrl(company.whatsapp)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-7 py-3.5 font-bold text-primary-foreground shadow-lg shadow-primary/25 transition hover:brightness-95">
        ...
        {mapUrl && (
          <div className="mt-12 overflow-hidden rounded-3xl border border-border shadow-lg">
            <iframe title="Localização da UTIL LOCADORA" src={mapUrl} loading="lazy" className="h-[320px] w-full border-0 md:h-[420px]" referrerPolicy="no-referrer-when-downgrade" />
          </div>
        )}
      </div>
    </section>
